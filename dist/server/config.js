    ...
    validator(config) {
        ...
        Object.keys(config.contentTypes).forEach((uid) => {
            ...
            if (!Array.isArray(config.contentTypes[uid])) {
                throw new utils_1.errors.ValidationError(`Must define delete protection rules as an array for ${uid}.`);
            }
            config.contentTypes[uid].forEach((rule) => {
                ...
                if (rule.length !== 3) {
                    throw new utils_1.errors.ValidationError(`Must define delete protection rules with all arguments for ${uid}.`);
                }
                ...
                if (!comparators.includes(comparator)) {
                    throw new utils_1.errors.ValidationError(`Unknown comparator ${comparator} used for ${uid} protection rules.`);
                }
                ...
                if (['in', 'notIn', 'between'].includes(comparator) && !Array.isArray(value)) {
                    throw new utils_1.errors.ValidationError(`Invalid ${comparator} value for ${uid} protection rules.`);
                }
                // Validate `between` comparators are compared against an array of 2 `values`.
                if (comparator === 'between' && value.length !== 2) {
                    throw new utils_1.errors.ValidationError(`Invalid number of array values for between comparators for ${uid} protection rules.`);
                }
                // Validate date comparators are compared against a valid date string.
                if (['day', 'month', 'year'].includes(comparator)) {
                    throw new utils_1.errors.ValidationError(`Invalid date string for ${comparator} value for ${uid} protection rules.`);
                }
            });
        });
    },
